...
export function getGuestStayStatus(reservation, today = new Date().toISOString().slice(0, 10)) {
...
export function getGuestAmountDue(entry, getAmountDue = reservation => Math.max(0, Number(reservation?.totalAmount || reservation?.amount || 0) - Number(reservation?.paidAmount || 0))) {
  return getAmountDue(entry?.reservation);
}

export function getGuestDocumentTypeList(entry, getDocumentLabel = value => value) {
  const docs = [];
...
export function getGuestRoomColorKey(entry) {
...
export function getGuestRoomSortKey(entry) {
...
export function buildGuestRoomColorMap(entries = []) {
  const rooms = new Map();
...
export function getGuestRoomColorIndex(entry, roomColorMap = null) {
  const key = getGuestRoomColorKey(entry);
  if (roomColorMap && roomColorMap.has(key)) return roomColorMap.get(key);
  return 0;
}

export function getGuestRoomColorClass(entry, roomColorMap = null) {
  const index = getGuestRoomColorIndex(entry, roomColorMap);
  return `guest-room-color-${(index % GUEST_ROOM_COLOR_CLASS_COUNT) + 1}`;
}

export function getGuestTypeSortRank(entry) {
  return GUEST_TYPE_SORT_ORDER[entry?.type] || 9;
}

export function compareGuestsByRoomGroup(a, b, roomColorMap) {
...
export function summarizeGuests(entries = [], getAmountDue = getGuestAmountDue, getDocuments = getGuestDocumentTypeList) {
...
export function filterGuests(entries = [], filters = {}) {
  const search = normalizeGuestText(filters.search || '');
  ...
  return entries.filter(entry => {
    ...
    const matchesSearch = !search || [
      ...
      entry.sourceLabel,
      docs
    ].some(value => normalizeGuestText(value).includes(search));
