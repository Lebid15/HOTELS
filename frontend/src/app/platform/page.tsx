    ...
    <div className="pf-section-head">
      ...
      {linkHref && (
        <Link href={linkHref} className="ds-btn ds-btn-neutral ds-btn-sm">
          <ArrowLeft size={13} strokeWidth={2} /> {linkLabel ?? t("عرض الكل")}
        </Link>
      )}
    </div>
  );
...
function EarningsSummaryCard() {
  ...
  return (
    <div className="ds-card-p">
      <SectionHead icon={<TrendingUp size={16} strokeWidth={2} />} title={t("أرباح المنصة")} linkHref="/platform/earnings" linkLabel={t("فتح تقرير الأرباح")} />
      {!d ? (
        <p className="pf-empty-inline">{t("جارٍ التحميل...")}</p>
      ) : (
        <div className="pf-grid-3">
          <div className="pf-fin-block">
            <p className="ds-summary-label">{t("أرباح الاشتراكات")}</p>
            <div className="pf-money-stack pf-money-big text-success"><MoneyLines map={d.subscription_earnings_by_currency} /></div>
          </div>
          <div className="pf-fin-block">
            <p className="ds-summary-label">{t("أرباح حجوزات الموقع")}</p>
          ...
          <div className="pf-fin-block pf-fin-total">
            <p className="ds-summary-label">{t("إجمالي أرباح المنصة")}</p>
...
export default function PlatformDashboard() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!res.ok) throw new Error(t("فشل تحميل لوحة التحكم"));
      setData(await res.json());
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : t("حدث خطأ غير متوقع"));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- تحميل بيانات المنصّة عند الإقلاع
    load();
    setPlatformCfg(readLocal<PlatformLocalSettings>("fandqi.platform", {
      platformName: "funduqii", platformEmail: "", platformPhone: "", platformLogo: "",
    }));
  ...
  async function quickApprove(reqId: number) {
    ...
    try {
      ...
      showToast(t("تم قبول الطلب بنجاح")); load();
    } catch { showToast(t("فشل قبول الطلب"), "error"); }
  ...
  async function quickReject(reqId: number) {
    ...
    try {
      ...
      if (!res.ok) throw new Error();
      showToast(t("تم رفض الطلب")); load();
    } catch { showToast(t("فشل رفض الطلب"), "error"); }
    finally { setActingId(null); }
  }
  ...
  const healthChecks = platformCfg ? [
    { label: t("اسم المنصة مخصص"),          ok: !!platformCfg.platformName && platformCfg.platformName !== "funduqii" },
    { label: t("البريد الإلكتروني للتواصل"), ok: !!platformCfg.platformEmail },
    { label: t("رقم الهاتف"),               ok: !!platformCfg.platformPhone },
    { label: t("شعار المنصة"),              ok: !!platformCfg.platformLogo },
    { label: t("مدة الفترة التجريبية"),      ok: !!subCfg && subCfg.trialDays > 0 },
    { label: t("أيام التذكير بالتجديد"),     ok: !!subCfg && subCfg.reminderDays > 0 },
  ] : [];
  ...
  if (kpis) {
    if (kpis.subscription_requests_pending > 0)
      alerts.push({ type: "warning", msg: `${t("يوجد")} ${kpis.subscription_requests_pending} ${t("طلب اشتراك معلق بانتظار المراجعة")}`, href: "/platform/subscription-requests" });
    if (kpis.subscriptions_ending_soon > 0)
      alerts.push({ type: "warning", msg: `${kpis.subscriptions_ending_soon} ${t("اشتراك تنتهي خلال 30 يوماً")}`, href: "/platform/subscriptions" });
    if (kpis.subscriptions_unpaid > 0)
      alerts.push({ type: "danger", msg: `${kpis.subscriptions_unpaid} ${t("اشتراك غير مدفوع أو مدفوع جزئياً")}`, href: "/platform/subscriptions" });
    if (kpis.hotels_suspended > 0)
      alerts.push({ type: "danger", msg: `${kpis.hotels_suspended} ${t("فندق موقوف يحتاج مراجعة")}`, href: "/platform/hotels" });
    if (web && web.cancelled_by_guest > 0)
      alerts.push({ type: "info", msg: `${web.cancelled_by_guest} ${t("حجز موقع أُلغي من الزبائن")}`, href: "/platform/web-bookings" });
    if (!healthOk && platformCfg)
      alerts.push({ type: "info", msg: t("إعدادات المنصة غير مكتملة — بعض الحقول تحتاج ضبطاً"), href: "/platform/settings" });
  }
  ...
  return (
    <div className="ds-page" dir="rtl">
      ...
      <div className="page-header">
        <div>
          <h1>{t("لوحة تحكم المنصة")}</h1>
          <p className="text-muted">
            {todayArabic()}
            {data?.platform_owner && (
              <> &mdash; {t("مرحباً،")} <strong className="pf-cell-title">{data.platform_owner.first_name || data.platform_owner.username}</strong></>
            )}
          </p>
        </div>
        <div className="page-actions">
          <a href="/" target="_blank" rel="noopener noreferrer" className="ds-btn ds-btn-teal ds-btn-sm">
            <Globe size={14} strokeWidth={2} /> {t("فتح الموقع العام")} <ExternalLink size={12} strokeWidth={2} />
          </a>
          <button className="ds-btn ds-btn-neutral ds-btn-sm" onClick={load} disabled={loading}>
            <RefreshCw size={14} strokeWidth={2} className={loading ? "spin" : ""} /> {t("تحديث")}
          </button>
          <Link href="/platform/settings" className="ds-btn ds-btn-neutral ds-btn-sm">
            <Settings size={14} strokeWidth={2} /> {t("الإعدادات")}
          </Link>
        </div>
      </div>

      {loading && <div className="ds-card-p"><p className="pf-empty-inline">{t("جارٍ تحميل لوحة التحكم...")}</p></div>}
      {error && (
        <div className="ds-alert ds-alert-danger pf-alert-row">
          <span><AlertTriangle size={15} strokeWidth={2.5} /> {error}</span>
          <button className="ds-btn ds-btn-danger ds-btn-sm" onClick={load}>{t("إعادة المحاولة")}</button>
        </div>
      )}
      ...
      {!loading && !error && data && (
        <>
            ...
            <Link href="/platform/hotels" className="pf-card-link">
              <div className="pf-cmd-tile">
                ...
                <div className="pf-cmd-body">
                  ...
                  <div className="pf-cmd-label">{t("إجمالي الفنادق")}</div>
                </div>
              </div>
            </Link>
            <Link href="/platform/subscriptions?status=active" className="pf-card-link">
              <div className="pf-cmd-tile">
                ...
                <div className="pf-cmd-body">
                  ...
                  <div className="pf-cmd-label">{t("اشتراكات فعالة")}</div>
                </div>
              </div>
            </Link>
            <Link href="/platform/subscription-requests" className="pf-card-link">
              <div className="pf-cmd-tile">
                ...
                <div className="pf-cmd-body">
                  ...
                  <div className="pf-cmd-label">{t("طلبات معلقة")}</div>
                </div>
              </div>
            </Link>
          </div>
          ...
          {/* ── Quick actions ──────────────────────────────────────── */}
          <div className="ds-card-p">
            <p className="pf-section-label">{t("إجراءات سريعة")}</p>
            <div className="pf-actions-row">
              <Link href="/platform/hotels" className="ds-btn ds-btn-primary ds-btn-sm"><Building2 size={13} /><Plus size={11} strokeWidth={3} /> {t("فندق جديد")}</Link>
              <Link href="/platform/managers" className="ds-btn ds-btn-neutral ds-btn-sm"><Users size={13} /> {t("المديرون")}</Link>
              <Link href="/platform/packages" className="ds-btn ds-btn-luxury ds-btn-sm"><Package size={13} /><Plus size={11} strokeWidth={3} /> {t("باقة جديدة")}</Link>
              <Link href="/platform/subscriptions" className="ds-btn ds-btn-teal ds-btn-sm"><CreditCard size={13} /><Plus size={11} strokeWidth={3} /> {t("اشتراك جديد")}</Link>
              <Link href="/platform/subscription-requests" className="ds-btn ds-btn-warning ds-btn-sm">
                <ClipboardList size={13} /> {t("طلبات الاشتراك")}
              ...
              <Link href="/platform/web-bookings" className="ds-btn ds-btn-edit ds-btn-sm"><CalendarCheck size={13} /> {t("حجوزات الموقع")}</Link>
              <Link href="/platform/earnings" className="ds-btn ds-btn-success ds-btn-sm"><TrendingUp size={13} /> {t("تقرير أرباحي")}</Link>
              <Link href="/platform/settings" className="ds-btn ds-btn-neutral ds-btn-sm"><Settings size={13} /> {t("الإعدادات")}</Link>
            </div>
          </div>

          {/* ── Smart alerts ───────────────────────────────────────── */}
          {alerts.length === 0 ? (
            <div className="ds-alert ds-alert-success"><CheckCircle2 size={16} strokeWidth={2.5} /> {t("كل شيء على ما يرام — لا تنبيهات مطلوبة حالياً.")}</div>
          ) : (
            <div className="pf-alert-stack">
              {alerts.map((a, i) => (
                <div key={i} className={`ds-alert ds-alert-${a.type} pf-alert-row`}>
                  ...
                  <Link href={a.href} className="ds-btn ds-btn-neutral ds-btn-sm"><ArrowLeft size={12} /> {t("معالجة")}</Link>
                </div>
              ))}
            </div>
          )}
          ...
          {/* ── KPI cluster: hotels ─────────────────────────────────── */}
          <p className="pf-cluster-label"><Building2 size={14} strokeWidth={2.5} /> {t("مؤشرات الفنادق")}</p>
          <div className="pf-grid-3">
            <KpiCard href="/platform/hotels" icon={<Building2 size={20} strokeWidth={2} />} tone="primary"
              label={t("إجمالي الفنادق")} value={kpis!.hotels_total} note={t("جميع الفنادق المسجلة في المنصة")} />
            <KpiCard href="/platform/hotels?status=active" icon={<CheckCircle2 size={20} strokeWidth={2} />} tone="success"
              label={t("فنادق فعالة")} value={kpis!.hotels_active} note={t("تعمل حالياً بشكل طبيعي")} />
            <KpiCard href="/platform/hotels?status=suspended" icon={<CircleSlash size={20} strokeWidth={2} />} tone="danger"
              label={t("فنادق موقوفة")} value={kpis!.hotels_suspended} note={t("موقوفة وتحتاج مراجعة")} />
          </div>

          {/* ── KPI cluster: subscriptions ──────────────────────────── */}
          <p className="pf-cluster-label"><CreditCard size={14} strokeWidth={2.5} /> {t("مؤشرات الاشتراكات")}</p>
          <div className="pf-grid-3">
            <KpiCard href="/platform/subscriptions?status=active" icon={<BadgeCheck size={20} strokeWidth={2} />} tone="success"
              label={t("اشتراكات فعالة")} value={kpis!.subscriptions_active} note={t("اشتراكات سارية ومدفوعة")} />
            <KpiCard href="/platform/subscriptions?status=trial" icon={<FlaskConical size={20} strokeWidth={2} />} tone="primary"
              label={t("اشتراكات تجريبية")} value={kpis!.subscriptions_trial} note={t("في فترة التجربة المجانية")} />
            <KpiCard href="/platform/subscriptions" icon={<Clock size={20} strokeWidth={2} />} tone="warning"
              label={t("تنتهي خلال 30 يوم")} value={kpis!.subscriptions_ending_soon} note={t("تحتاج تجديداً قريباً")} />
            <KpiCard href="/platform/subscriptions?status=expired" icon={<XCircle size={20} strokeWidth={2} />} tone="danger"
              label={t("اشتراكات منتهية")} value={kpis!.subscriptions_expired} note={t("منتهية وتحتاج تجديد أو إغلاق")} />
            <KpiCard href="/platform/subscriptions" icon={<Wallet size={20} strokeWidth={2} />} tone="warning"
              label={t("فنادق بلا اشتراك")} value={kpis!.hotels_without_subscription} note={t("لم يُنشأ لها اشتراك بعد")} />
            <KpiCard href="/platform/subscription-requests" icon={<Hourglass size={20} strokeWidth={2} />} tone="accent"
              label={t("طلبات معلقة")} value={kpis!.subscription_requests_pending} note={t("طلبات بانتظار المراجعة")} />
          </div>
          ...
          <div className="pf-two-col">
            ...
            <div className="ds-card-p">
              <SectionHead icon={<CalendarCheck size={16} strokeWidth={2} />} title={t("حجوزات الموقع الحديثة")} linkHref="/platform/web-bookings" linkLabel={t("كل الحجوزات")} />
              {web && (
                <div className="pf-stat-strip">
                  <div className="pf-stat-pill"><div className="pf-stat-num">{web.today}</div><div className="pf-stat-cap">{t("اليوم")}</div></div>
                  <div className="pf-stat-pill"><div className="pf-stat-num">{web.month}</div><div className="pf-stat-cap">{t("هذا الشهر")}</div></div>
                  <div className="pf-stat-pill"><div className="pf-stat-num">{web.awaiting}</div><div className="pf-stat-cap">{t("بانتظار الوصول")}</div></div>
                  <div className="pf-stat-pill"><div className="pf-stat-num">{web.cancelled_by_guest}</div><div className="pf-stat-cap">{t("ملغاة من الزبون")}</div></div>
                  <div className="pf-stat-pill"><div className="pf-stat-num">{web.no_show}</div><div className="pf-stat-cap">{t("لم يحضر")}</div></div>
                </div>
              )}
              {data.recent_web_bookings.length === 0 ? (
                <p className="pf-empty-inline">{t("لا توجد حجوزات من الموقع بعد.")}</p>
              ) : (
                <div className="ds-table-wrap">
                  <table className="ds-table">
                    <thead><tr><th>{t("رقم الحجز")}</th><th>{t("الفندق")}</th><th>{t("الزبون")}</th><th>{t("القيمة")}</th><th>{t("الحالة")}</th><th>{t("التاريخ")}</th></tr></thead>
                    <tbody>
                      {data.recent_web_bookings.map(b => (
                        <tr key={b.id}>
                          ...
                          <td><span className={RES_BADGE[b.status] ?? "ds-badge ds-badge-neutral"}>{RES_LABEL[b.status] ? t(RES_LABEL[b.status]) : b.status}</span></td>
            ...
            <div className="ds-card-p">
              <SectionHead icon={<TrendingUp size={16} strokeWidth={2} />} title={t("ملخص الإيرادات (حسب العملة)")} linkHref="/platform/subscriptions" linkLabel={t("الاشتراكات")} />
              {data.revenue.length === 0 ? (
                <p className="pf-empty-inline">{t("لا توجد بيانات إيرادات حتى الآن.")}</p>
              ) : (
                <div className="pf-revenue-grid">
                  {data.revenue.map(r => (
                    <div key={r.currency} className="pf-revenue-card">
                      <p className="pf-revenue-title">{t(currencyLabel(r.currency))} ({r.currency})</p>
                      <div className="pf-revenue-row">
                        <span className="text-muted">{t("مدفوع")} ({r.count_paid} {t("اشتراك")})</span>
                        <span className="text-success pf-cell-title">{fmt(r.total_paid)} {r.currency}</span>
                      </div>
                      <div className="pf-revenue-row">
                        <span className="text-muted">{t("غير مدفوع")} ({r.count_unpaid} {t("اشتراك")})</span>
          ...
          <div className="ds-card-p">
            <SectionHead icon={<FileText size={16} strokeWidth={2} />} title={t("أحدث طلبات الاشتراك")} count={data.recent_requests.length} linkHref="/platform/subscription-requests" linkLabel={t("إدارة الطلبات")} />
            {data.recent_requests.length === 0 ? (
              <p className="pf-empty-inline">{t("لا توجد طلبات حتى الآن.")}</p>
            ) : (
              <div className="ds-table-wrap">
                <table className="ds-table">
                  <thead><tr><th>{t("الفندق")}</th><th>{t("الباقة")}</th><th>{t("مقدم الطلب")}</th><th>{t("الحالة")}</th><th>{t("التاريخ")}</th><th>{t("إجراء سريع")}</th></tr></thead>
                  <tbody>
                    {data.recent_requests.map(req => (
                      <tr key={req.id}>
                        ...
                        <td><span className={REQ_BADGE[req.status] ?? "ds-badge ds-badge-neutral"}>{REQ_LABEL[req.status] ? t(REQ_LABEL[req.status]) : req.status}</span></td>
                        <td className="text-muted">{shortDate(req.created_at)}</td>
                        <td>
                          {req.status === "pending" ? (
                            <div className="table-actions">
                              <button className="ds-btn ds-btn-success ds-btn-sm" onClick={() => quickApprove(req.id)} disabled={actingId === req.id}>{t("قبول")}</button>
                              <button className="ds-btn ds-btn-danger ds-btn-sm" onClick={() => quickReject(req.id)} disabled={actingId === req.id}>{t("رفض")}</button>
                            </div>
          ...
          {data.ending_soon.length > 0 && (
            <div className="ds-card-p">
              <SectionHead icon={<Clock size={16} strokeWidth={2} />} title={t("اشتراكات تنتهي قريباً")} count={data.ending_soon.length} linkHref="/platform/subscriptions" linkLabel={t("إدارة الاشتراكات")} />
              <div className="ds-table-wrap">
                <table className="ds-table">
                  <thead><tr><th>{t("الفندق")}</th><th>{t("الباقة")}</th><th>{t("تاريخ الانتهاء")}</th><th>{t("المتبقي")}</th><th>{t("المبلغ")}</th><th>{t("الدفع")}</th><th>{t("الحالة")}</th></tr></thead>
                  <tbody>
                    {data.ending_soon.map(sub => (
                      <tr key={sub.id}>
                        ...
                        <td>{sub.remaining_days !== null ? (
                          <span className={`ds-badge ${sub.remaining_days <= 7 ? "ds-badge-danger" : "ds-badge-warning"}`}>{sub.remaining_days} {t("يوم")}</span>
                        ) : "—"}</td>
                        <td>{sub.monthly_amount > 0 ? `${fmt(sub.monthly_amount)} ${sub.currency}` : <span className="text-muted">—</span>}</td>
                        <td><span className={PAY_BADGE[sub.payment_status] ?? "ds-badge ds-badge-neutral"}>{PAY_LABEL[sub.payment_status] ? t(PAY_LABEL[sub.payment_status]) : sub.payment_status}</span></td>
                        <td><span className={SUB_BADGE[sub.status] ?? "ds-badge ds-badge-neutral"}>{SUB_LABEL[sub.status] ? t(SUB_LABEL[sub.status]) : sub.status}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="pf-two-col">
            <div className="ds-card-p">
              <SectionHead icon={<BarChart3 size={16} strokeWidth={2} />} title={t("توزيع حالة الفنادق")} linkHref="/platform/hotels" linkLabel={t("إدارة الفنادق")} />
              {bd && kpis && kpis.hotels_total > 0 ? (
                <div className="pf-progress-list">
                  {[
                    { label: t("فعالة"), value: bd.active, color: "var(--color-success)" },
                    { label: t("تجريبية"), value: bd.trial, color: "var(--color-primary)" },
                    { label: t("موقوفة"), value: bd.suspended, color: "var(--color-danger)" },
                    { label: t("منتهي الاشتراك"), value: bd.expired, color: "var(--color-warning)" },
                    { label: t("بلا اشتراك"), value: bd.without_subscription, color: "var(--color-muted)" },
                  ].map(row => (
                    <div key={row.label}>
              ...
              ) : <p className="pf-empty-inline">{t("لا توجد فنادق مسجلة.")}</p>}
            </div>

            <div className="ds-card-p">
              <SectionHead icon={<Building2 size={16} strokeWidth={2} />} title={t("أحدث الفنادق المضافة")} count={data.recent_hotels.length} linkHref="/platform/hotels" linkLabel={t("جميع الفنادق")} />
              {data.recent_hotels.length === 0 ? (
                <p className="pf-empty-inline">{t("لا توجد فنادق.")}</p>
              ) : (
                <div className="ds-table-wrap">
                  <table className="ds-table">
                    <thead><tr><th>{t("الفندق")}</th><th>{t("الحالة")}</th><th>{t("الاشتراك")}</th></tr></thead>
                    <tbody>
                      {data.recent_hotels.map(h => (
                        <tr key={h.id}>
                          ...
                          <td><span className={HOTEL_BADGE[h.status] ?? "ds-badge ds-badge-neutral"}>{HOTEL_LABEL[h.status] ? t(HOTEL_LABEL[h.status]) : h.status}</span></td>
                          <td>{h.subscription_status ? (
                            <span className={SUB_BADGE[h.subscription_status] ?? "ds-badge ds-badge-neutral"}>{SUB_LABEL[h.subscription_status] ? t(SUB_LABEL[h.subscription_status]) : h.subscription_status}</span>
          ...
          {data.package_distribution.length > 0 && (
            <div className="ds-card-p">
              <SectionHead icon={<Layers size={16} strokeWidth={2} />} title={t("توزيع الباقات")} count={data.package_distribution.length} linkHref="/platform/packages" linkLabel={t("إدارة الباقات")} />
              <div className="pf-progress-list">
                  ...
                  <div key={pkg.id} className="pf-pkg-row">
                    <div className="pf-pkg-name">
                      ...
                      <div className="pf-pkg-meta">
                        <span className={PKG_STATUS_BADGE[pkg.status] ?? "ds-badge ds-badge-neutral"}>{PKG_STATUS_LABEL[pkg.status] ? t(PKG_STATUS_LABEL[pkg.status]) : pkg.status}</span>
                        {pkg.price_monthly !== null && <span className="pf-mini-meta">{fmt(pkg.price_monthly)}/{t("شهر")}</span>}
                      </div>
                    </div>
                    ...
                    <span className="pf-mini-meta">{t("اشتراك")}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {healthChecks.length > 0 && (
            <div className="ds-card-p">
              <SectionHead icon={<Settings size={16} strokeWidth={2} />} title={t("صحة إعدادات المنصة")} linkHref="/platform/settings" linkLabel={t("فتح الإعدادات")} />
              {healthOk ? (
                <div className="ds-alert ds-alert-success"><CheckCircle2 size={15} strokeWidth={2.5} /> {t("جميع إعدادات المنصة مكتملة ومضبوطة.")}</div>
              ) : (
                <div className="ds-alert ds-alert-warning"><AlertTriangle size={15} strokeWidth={2.5} /> {t("بعض الإعدادات تحتاج إلى ضبط لتكتمل هوية المنصة.")}</div>
              )}
