  ...
  return (
    <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
      {entries.map(([st, c]) => {
        ...
        return <span key={st} className="earn-mini-badge" style={{ background: s.bg, color: s.fg }}>{t(s.label)} {c}</span>;
      })}
    </div>
  );
}
...
export default function EarningsPage() {
  ...
  const load = useCallback(() => {
    ...
    fetch(apiUrl(`/platform/earnings/?${buildQuery()}`), { headers: getAuthJsonHeaders() })
      ...
      .catch(() => { setError(t("تعذّر تحميل تقرير الأرباح")); setLoading(false); });
  }, [buildQuery, t]);
  ...
  function saveSettings() {
    ...
    fetch(apiUrl("/platform/revenue-settings/"), {
      ...
      .then(r => r.ok ? r.json() : Promise.reject())
      .then(() => { setSavingSettings(false); setShowSettings(false); showToast(t("تم حفظ الإعدادات")); load(); })
      .catch(() => { setSavingSettings(false); showToast(t("فشل حفظ الإعدادات")); });
  }
  ...
  if (loading && !data) {
    return <div className="ds-page" dir="rtl"><div className="ds-card-p"><p className="text-muted">{t("جارٍ تحميل تقرير الأرباح...")}</p></div></div>;
  }
  if (error || !data) {
    return <div className="ds-page" dir="rtl"><div className="ds-card-p"><div className="ds-alert ds-alert-danger">{error || t("خطأ")}</div></div></div>;
  }
  ...
  return (
    <div className="ds-page" dir="rtl">
      ...
      <div className="print-only" style={{ marginBottom: "1rem", textAlign: "center" }}>
        <h2 style={{ fontWeight: 800, fontSize: 22 }}>{t("تقرير أرباح المنصة")}</h2>
        <p style={{ color: "#555" }}>{t("الفترة")}: {t(periodLabel)} · {t("تاريخ الإنشاء")}: {new Date().toLocaleDateString("ar-SY")}</p>
      </div>

      {/* Header */}
      <div className="page-header">
        <div>
          <h1 style={{ display: "flex", alignItems: "center", gap: 8 }}><TrendingUp size={24} /> {t("أرباحي")}</h1>
          <p>{t("مركز متابعة دخل المنصة من الاشتراكات وحجوزات الموقع")}</p>
        </div>
        <div className="page-actions">
          ...
          <button className="ds-btn ds-btn-primary ds-btn-sm" onClick={() => setShowSettings(true)} style={{ gap: 6 }}>
            <SettingsIcon size={15} /> {t("إعدادات العمولة")}
          </button>
        </div>
      </div>
      ...
      <div className="ds-filters no-print" style={{ marginBottom: "1.25rem" }}>
        ...
        <select className="select" value={govFilter} onChange={e => setGovFilter(e.target.value)} style={{ width: 150 }}>
          <option value="">{t("كل المحافظات")}</option>
        ...
        <div className="ds-tabs" style={{ marginRight: "auto" }}>
          {([["all", "الكل"], ["subscriptions", "اشتراكات"], ["web_bookings", "حجوزات الموقع"]] as const).map(([k, l]) => (
            <button key={k} className={`ds-tab-btn${profitType === k ? " active" : ""}`} onClick={() => setProfitType(k)}>{t(l)}</button>
          ))}
        </div>
      </div>
      ...
      <div className="earn-cards">
        <div className="ds-summary-card">
          <p className="ds-summary-label"><Building2 size={14} style={{ display: "inline", marginLeft: 4 }} />{t("إجمالي الفنادق")}</p>
          <div className="ds-summary-value">{data.kpis.hotels_total}</div>
          <p className="ds-summary-note">{t("كل الفنادق المسجّلة")}</p>
        </div>
        <div className="ds-summary-card">
          <p className="ds-summary-label"><Building2 size={14} style={{ display: "inline", marginLeft: 4 }} />{t("الفنادق النشطة")}</p>
          <div className="ds-summary-value text-success">{data.kpis.hotels_active}</div>
          <p className="ds-summary-note">{t("فنادق فعّالة")}</p>
        </div>
        <div className="ds-summary-card">
          <p className="ds-summary-label"><BadgeCheck size={14} style={{ display: "inline", marginLeft: 4 }} />{t("الاشتراكات المباعة")}</p>
          <div className="ds-summary-value">{data.kpis.subscriptions_sold}</div>
          <p className="ds-summary-note">{t("اشتراكات مدفوعة")}</p>
        </div>

        {showSubs && (
          <div className="ds-summary-card">
            <p className="ds-summary-label"><CircleDollarSign size={14} style={{ display: "inline", marginLeft: 4 }} />{t("أرباح الاشتراكات")}</p>
            <div className="ds-summary-value text-success"><MoneyLines map={data.subscription_earnings_by_currency} empty="0" /></div>
            <p className="ds-summary-note">{t("من الاشتراكات المدفوعة")}</p>
          </div>
        )}
        {showWeb && (
          <div className="ds-summary-card">
            <p className="ds-summary-label"><CalendarCheck size={14} style={{ display: "inline", marginLeft: 4 }} />{t("حجوزات الموقع")}</p>
            <div className="ds-summary-value">{data.web_bookings.count}</div>
            <p className="ds-summary-note">{t("حجوزات من الموقع العام")}</p>
          </div>
        )}
        {showWeb && (
          <div className="ds-summary-card">
            <p className="ds-summary-label"><Users size={14} style={{ display: "inline", marginLeft: 4 }} />{t("زبائن حجزوا عبر موقعنا")}</p>
            <div className="ds-summary-value text-primary">{data.web_bookings.customers}</div>
            <p className="ds-summary-note">{t("زبائن فريدون (حسب الهاتف)")}</p>
          </div>
        )}
        {showWeb && (
          <div className="ds-summary-card">
            <p className="ds-summary-label"><HandCoins size={14} style={{ display: "inline", marginLeft: 4 }} />{t("أرباح حجوزات الموقع")}</p>
            <div className="ds-summary-value text-success"><MoneyLines map={data.booking_earnings_by_currency} empty="0" /></div>
            <p className="ds-summary-note">{t("عمولات مستحقة + مدفوعة")}</p>
          </div>
        )}
        <div className="ds-summary-card" style={{ borderColor: "var(--color-primary)", borderWidth: 2 }}>
          <p className="ds-summary-label"><TrendingUp size={14} style={{ display: "inline", marginLeft: 4 }} />{t("إجمالي أرباح المنصة")}</p>
          <div className="ds-summary-value text-primary"><MoneyLines map={data.total_by_currency} empty="0" /></div>
          <p className="ds-summary-note">{t("اشتراكات + حجوزات الموقع")}</p>
        </div>
      </div>
      ...
      {showSubs && (
        <>
          <h2 className="earn-section-title"><CircleDollarSign size={18} /> {t("أرباح الاشتراكات")}</h2>
          <div className="ds-card ds-card-p" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: "1rem" }}>
            <Stat label={t("مباعة")} value={data.subscriptions.sold} />
            <Stat label={t("نشطة")} value={data.subscriptions.active} color="text-success" />
            <Stat label={t("منتهية")} value={data.subscriptions.expired} color="text-danger" />
            <Stat label={t("غير مدفوعة")} value={data.subscriptions.unpaid} color="text-warning" />
            <Stat label={t("تجريبية")} value={data.subscriptions.trial} />
            <div>
              <p className="ds-summary-label">{t("غير المحصّل")}</p>
              <div style={{ fontWeight: 800, fontSize: "var(--text-lg)" }}><MoneyLines map={data.subscriptions.unpaid_by_currency} empty="0" /></div>
            </div>
            <div>
              <p className="ds-summary-label">{t("أفضل باقة مبيعًا")}</p>
              <div style={{ fontWeight: 700 }}>{data.subscriptions.best_selling_package ?? "—"}</div>
            </div>
            <div>
              <p className="ds-summary-label">{t("أكثر فندق دفعًا")}</p>
      ...
      {showWeb && (
        <>
          <h2 className="earn-section-title"><CalendarCheck size={18} /> {t("أرباح حجوزات الموقع")}</h2>
          <div className="ds-card ds-card-p" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: "1rem" }}>
            <Stat label={t("إجمالي الحجوزات")} value={data.web_bookings.count} />
            <Stat label={t("مكتملة")} value={data.web_bookings.completed} color="text-success" />
            <Stat label={t("ملغاة")} value={data.web_bookings.cancelled} color="text-danger" />
            <Stat label={t("لم يحضر")} value={data.web_bookings.no_show} color="text-warning" />
            <Stat label={t("فنادق لها حجوزات")} value={data.web_bookings.hotels_with_bookings} />
            <div>
              <p className="ds-summary-label"><CircleCheck size={12} style={{ display: "inline", marginLeft: 3 }} />{t("عمولات مدفوعة")}</p>
            ...
            <div>
              <p className="ds-summary-label"><AlertCircle size={12} style={{ display: "inline", marginLeft: 3 }} />{t("عمولات قيد الانتظار")}</p>
      ...
      {/* Hotels report table */}
      <h2 className="earn-section-title"><FileBarChart size={18} /> {t("تقرير أرباح الفنادق")}</h2>
        ...
        <table className="ds-table">
          <thead>
            <tr>
              ...
              <th>{t("الباقة")}</th>
              <th>{t("الاشتراك")}</th>
              <th>{t("حجوزات الموقع")}</th>
              <th>{t("مكتملة")}</th>
              <th>{t("ملغاة")}</th>
              <th>{t("لم يحضر")}</th>
              <th>{t("قيمة الحجوزات")}</th>
              <th>{t("العمولة")}</th>
              <th>{t("ربح المنصة")}</th>
              <th>{t("حالة التحصيل")}</th>
              <th>{t("آخر حجز")}</th>
              <th></th>
            </tr>
          </thead>
          <tbody>
            {data.hotels.map(h => (
              <tr key={h.hotel_id}>
                ...
                <td><span className="earn-mini-badge" style={{ background: h.status === "active" ? "var(--color-success-soft)" : "var(--color-border)", color: h.status === "active" ? "var(--color-success)" : "var(--color-muted)" }}>{HOTEL_STATUS_LABEL[h.status] ? t(HOTEL_STATUS_LABEL[h.status]) : h.status}</span></td>
                ...
                <td style={{ fontSize: 12 }}>
                  {h.commission_enabled
                    ? <>{t(COMMISSION_TYPE_LABEL[h.commission_type])}<br /><strong>{h.commission_type === "percentage" ? `${h.commission_value}%` : `${nf(h.commission_value)} ${h.commission_currency}`}</strong>{h.commission_source === "hotel" && <span className="earn-mini-badge" style={{ background: "var(--color-primary-soft)", color: "var(--color-primary)", marginRight: 4 }}>{t("خاص")}</span>}</>
                    : <span style={{ color: "var(--color-muted)" }}>{t("معطّلة")}</span>}
                </td>
                ...
                <td>
                  <Link href={`/platform/earnings/${h.hotel_id}`} className="ds-btn ds-btn-neutral ds-btn-xs" style={{ gap: 4, whiteSpace: "nowrap" }}>
                    <FileBarChart size={13} /> {t("التقرير")}
                  </Link>
                </td>
              </tr>
            ))}
            {data.hotels.length === 0 && (
              <tr><td colSpan={15} style={{ textAlign: "center", padding: "2rem", color: "var(--color-muted)" }}>{t("لا توجد فنادق مطابقة للفلاتر")}</td></tr>
            )}
          </tbody>
        </table>
      </div>
        ...
        <div className="ds-modal-backdrop" onClick={() => setShowSettings(false)}>
          <div className="ds-modal-card" style={{ maxWidth: 560 }} onClick={e => e.stopPropagation()}>
            <div className="ds-modal-head">
              <h2 style={{ display: "flex", alignItems: "center", gap: 8 }}><Percent size={18} /> {t("إعدادات احتساب ربح حجوزات الموقع")}</h2>
            ...
            <div className="ds-modal-body" style={{ display: "grid", gap: "1rem" }}>
              <label style={{ display: "flex", alignItems: "center", gap: 8, fontWeight: 600 }}>
                ...
                {t("تفعيل احتساب عمولة حجوزات الموقع")}
              </label>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                <div className="field">
                  <label className="field-label">{settings.default_commission_type === "percentage" ? t("النسبة (%)") : t("القيمة")}</label>
                ...
                <div className="field">
                  <label className="field-label">{t("عملة المبلغ المقطوع")}</label>
              ...
              <div className="field">
                <label className="field-label">{t("متى تُحتسب العمولة كمستحقة؟")}</label>
                <select className="select" value={settings.calculate_commission_on_status}
                  ...
                  {Object.entries(TRIGGER_LABEL).map(([k, v]) => <option key={k} value={k}>{t(v)}</option>)}
                </select>
                <p style={{ fontSize: 12, color: "var(--color-muted)", marginTop: 4 }}>{t("الأفضل: عند تسجيل الدخول أو الاكتمال، لأن الدفع يتم عند الوصول.")}</p>
              </div>

              <div className="field">
                <label className="field-label">{t("سياسة عدم الحضور (no-show)")}</label>
                <select className="select" value={settings.no_show_policy}
                  onChange={e => setSettings({ ...settings, no_show_policy: e.target.value })}>
                  <option value="waive">{t("إعفاء العمولة عند عدم الحضور")}</option>
                  <option value="keep">{t("إبقاء العمولة مستحقة")}</option>
                </select>
              </div>
              ...
              <label style={{ display: "flex", alignItems: "center", gap: 8, fontWeight: 600 }}>
                ...
                {t("السماح بإعداد عمولة خاص لكل فندق")}
              </label>
            </div>
            <div className="ds-modal-foot">
              <button className="ds-btn ds-btn-neutral" onClick={() => setShowSettings(false)}>{t("إلغاء")}</button>
              <button className="ds-btn ds-btn-primary" onClick={saveSettings} disabled={savingSettings}>
                {savingSettings ? t("جارٍ الحفظ...") : t("حفظ الإعدادات")}
              </button>
            </div>
          </div>
        </div>
